...
function handleKeyDown(e) {
  ...
  if (key === 'Delete') {
    e.preventDefault();
    if (selectedCell) {
      // Check if the cell has a solid black number (preset number)
      if (selectedCell.classList.contains('preset') && selectedCell.style.color === 'black') {
        // Do nothing if it's a solid black number (preset number)
        return;
      }
...
function selectCell(cell, isMultiSelect = false, isDrag = false) {
  ...
  if (hasMultipleSelections && !isCtrlMultiSelectMode && !isMultiSelectMode && !isDrag && !cell.classList.contains('selected')) {
    // Clear all selections and select only the clicked cell
...
for (let row = 0; row < 9; row++) {
  for (let col = 0; col < 9; col++) {
    ...
    // Click event - handle single/double click
    cell.addEventListener('click', (e) => {
      ...
      // Check if we're coming from YBG with all red stripes
      const ybgCell = document.querySelector('.ybg');
      const isFromYBG = ybgCell && ybgCell.classList.contains('selected');
      const allStripesFilled = ybgCell && ybgCell.querySelectorAll('.red-stripe').length >= 2;
      
      // Store the pending cell
      pendingCell = cell;
      
      // If coming from YBG with all stripes filled, wait to see if it's a double click
      if (isFromYBG && allStripesFilled) {
        if (clickTimer) {
          // This is the second click - handle as double click
          clearTimeout(clickTimer);
          clickTimer = null;
          
          // Move to WBG and handle double click
          ...
          clickTimer = setTimeout(() => {
            // Timer expired, handle as single click
            if (pendingCell) {
              selectCell(pendingCell, false, false);
              pendingCell = null;
            }
      ...
      // Normal click handling for all other cases
      if (clickTimer) {
        clearTimeout(clickTimer);
      }
      
      clickTimer = setTimeout(() => {
        // Clear any existing highlights if needed
        const highlightedCells = document.querySelectorAll('.highlight-same-number');
        if (highlightedCells.length > 0 && !cell.classList.contains('highlight-same-number')) {
          clearHighlights();
          isHighlighted = false;
        }
        ...
        // Remove any existing subcell highlights
        const existingHighlights = cell.querySelectorAll('.subcell-highlight');
...
// Function to clear all number highlights
function clearHighlights() {
  document.querySelectorAll('.cell.highlight-same-number').forEach(cell => {
...
function highlightSameNumberCells(clickedCell, fromDoubleClick = false) {
  // Get the solid number from the clicked cell (only highlight based on solid numbers)
  const cellText = clickedCell.textContent.trim();
  if (!cellText || cellText === '' || !/^[1-9]$/.test(cellText)) return;
  ...
  // Only change selection if not from a double-click
  if (!fromDoubleClick) {
    selectCell(clickedCell);
  }
  
  // Find and highlight all cells that contain the same number (solid or pencil marks)
  const allCells = document.querySelectorAll('.cell');
  allCells.forEach(cell => {
    if (cell !== clickedCell) {
      const cellNumbers = getAllNumbersFromCell(cell);
      if (cellNumbers.includes(cellText)) {
...
// Add event listener for del3Btn (red X button) to prevent deleting preset numbers
document.addEventListener('DOMContentLoaded', function() {
    ...
    newDel3Btn.addEventListener('click', function(e) {
        ...
        console.log('Selected cell:', {
            ...
            styleColor: selectedCell.style.color,
            computedColor: window.getComputedStyle(selectedCell).color,
            hasPresetClass: selectedCell.classList.contains('preset')
        });
        
        // Check if this is a preset cell
        ...
        console.log('Is preset?', isPreset);
        
        if (isPreset) {
            console.log('Preventing deletion of preset number');
            // Visual feedback
        ...
        // Proceed with normal delete behavior for non-preset cells
        const beforeState = saveCellState(selectedCell);
        selectedCell.textContent = '';
        selectedCell.style.color = '';
        selectedCell.style.fontWeight = '';
        selectedCell.style.fontSize = '';
        selectedCell.classList.remove('preset');
